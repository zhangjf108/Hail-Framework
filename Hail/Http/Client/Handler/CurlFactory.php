...
use Hail\Http\Client\Exception\ConnectException;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx)
    {
        ...
        if ($easy->onHeadersException) {
            return PromiseFactory::rejection(
        ...
        return PromiseFactory::rejection($error);
    }
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf)
    {
        ...
        if (isset($options['sink'])) {
            ...
            $easy->sink = HttpFactory::stream($conf[CURLOPT_FILE]);
        }
        ...
        if ($timeoutRequiresNoSignal && stripos(PHP_OS, 'WIN') !== 0) {
            ...
            if (!is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) ||
                        !Helpers::isHostInNoproxy($host, $options['proxy']['no'])
                    ) {
        ...
        if (!empty($options['debug'])) {
            $conf[CURLOPT_STDERR] = Helpers::debugResource($options['debug']);
