...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return PromiseFactory::rejection($e);
        }
    }
    ...
    private function createResponse(
        ...
        $response = HttpFactory::response($status, $sink, $headers, $ver, $reason);
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = Helpers::normalizeHeaderKeys($headers);
            if (isset($normalizedKeys['content-encoding'])) {
                $encoding = $headers[$normalizedKeys['content-encoding']];
                if ($encoding[0] === 'gzip' || $encoding[0] === 'deflate') {
                    $stream = new InflateStream(
                        HttpFactory::stream($stream)
                    );
    ...
    private function drain(
        ...
        \Hail\Http\Helpers::copyToStream(
            $source,
            $sink,
    ...
    private function createStream(RequestInterface $request, array $options)
    {
        ...
        if (isset($options['auth'], $options['auth'][2])
