...
class BlockMacros extends MacroInterfaceSet
{
    ...
    public function macroIncludeBlock(MacroNode $node, PhpWriter $writer)
    {
        ...
        return $writer->write(
            'ob_start(); $this->createTemplate(%node.word, %node.array? + get_defined_vars(), "includeblock")->renderToContentType(%var); echo rtrim(ob_get_clean());',
    ...
    public function macroBlock(MacroNode $node, PhpWriter $writer)
    {
        ...
        if ($node->name === 'snippet') {
            if ($node->prefix) {
                if (isset($node->htmlNode->macroAttrs['foreach'])) {
                    trigger_error('Combination of n:snippet with n:foreach is invalid, use n:inner-foreach.',
                        E_USER_WARNING);
                }
                $node->attrCode = $writer->write('<?php echo \' id="\' . htmlSpecialChars($this->global->snippetDriver->getHtmlId(%var)) . \'"\' ?>',
        ...
        if ($node->name === 'define') {
            ...
            if ($args) {
                $node->data->args = 'list(' . implode(', ', $args) . ') = $_args + [' . str_repeat('NULL, ',
                        count($args)) . '];';
            }
    ...
    public function macroBlockEnd(MacroNode $node, PhpWriter $writer)
    {
        if (isset($node->data->name)) { // block, snippet, define
            ...
            if (($node->name === 'snippet' || $node->name === 'snippetArea') && strpos($node->data->name,
                    '$') === false
            ) {
        ...
        if ($node->modifiers) { // anonymous block with modifier
            $node->modifiers .= '|escape';

            return $writer->write('$_fi = new LR\FilterInfo(%var); echo %modifyContent(ob_get_clean());',
                $node->context[0]);
        }
    }
