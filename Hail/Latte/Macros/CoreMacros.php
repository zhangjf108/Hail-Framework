...
class CoreMacros extends MacroSet
{
    ...
    public static function install(Compiler $compiler)
    {
        ...
        $me->addMacro('switch', '$this->global->switch[] = (%node.args); if (FALSE) {',
    ...
    public function finalize(): ?array
    {
        ...
        foreach ($this->overwrittenVars as $var => $lines) {
            ...
            $code .= 'if (isset($this->params[' . var_export($var, true)
                . "])) trigger_error('Variable $" . addcslashes($var,
                    "'") . ' overwritten in foreach on line ' . implode(', ', $lines) . "'); ";
        }
    ...
    public function macroTranslate(MacroNode $node, PhpWriter $writer)
    {
        if ($node->closing) {
            if (strpos($node->content, '<?php') === false) {
                ...
                $node->openingCode = '<?php ob_start() ?>' . $node->openingCode;
                $value = 'ob_get_clean()';
            }

            return $writer->write('$_fi = new LR\FilterInfo(%var); echo %modifyContent($this->filters->filterContent("translate", $_fi, %raw))',
                $node->context[0], $value);
    ...
    public function macroCaptureEnd(MacroNode $node, PhpWriter $writer)
    {
        ...
        return $writer->write("\$_fi = new LR\\FilterInfo(%var); %raw = %modifyContent($body);", $node->context[0],
            $node->data->variable);
    }
    ...
    public function macroEndForeach(MacroNode $node, PhpWriter $writer)
    {
        ...
        if (!$noCheck) {
            ...
            for ($i = 1, $max = count($m); $i < $max; $i++) {
                $this->overwrittenVars[$m[$i]][] = $node->startLine;
            }
        }
        if (!$noIterator && preg_match('#\W(\$iterator|include|require|get_defined_vars)\W#',
                $this->getCompiler()->expandTokens($node->content))
        ) {
    ...
    public function macroContentType(MacroNode $node, PhpWriter $writer)
    {
        if (!$this->getCompiler()->isInHead()
            && !($node->htmlNode && strtolower($node->htmlNode->name) === 'script' && strpos($node->args,
                    'html') !== false)
        ) {
