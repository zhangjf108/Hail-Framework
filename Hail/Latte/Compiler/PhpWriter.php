...
class PhpWriter
{
    ...
    {
        return (is_numeric($s) || preg_match('#^\$|[\'"]|^(true|TRUE)\z|^(false|FALSE)\z|^(null|NULL)\z|^[\w\\\\]{3,}::[A-Z0-9_]{2,}\z#',
                $s))
    ...
    public function validateTokens(MacroTokens $tokens)
    {
        ...
        while ($tokens->nextToken()) {
            ...
            } elseif (($tokens->isCurrent('return', 'yield') && !$brackets) ||
                (
            ...
            if ($tokens->isCurrent($tokens::T_VARIABLE)) {
                ...
                if ($depth === $tokens->depth && $tokens->nextValue('in') && ($arr[] = $tokens->nextToken('['))) {
                    while ($tokens->isNext()) {
                        ...
                        if ($tokens->depth === $depth && $tokens->isCurrent(']')) {
                            $new = array_merge($tokens->parse('in_array('), $expr, $tokens->parse(', '), $arr,
                                $tokens->parse(', TRUE)'));
            ...
            if ($tokens->isCurrent($tokens::T_WHITESPACE)) {
                ...
                if ($tokens->isCurrent($tokens::T_SYMBOL)) {
                    if ($tokens->isCurrent('escape')) {
                        if ($isContent) {
                            $res->prepend('LR\Filters::convertTo($_fi, ' . var_export(implode($this->context),
                                    true) . ', ')
                                ->append(')');
                        } else {
