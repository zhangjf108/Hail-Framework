...
class Parser
{
    ...
    public function parse(string $input): array
    {
        ...
        if (!preg_match('##u', $input)) {
            preg_match('#(?:[\x00-\x7F]|[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF7][\x80-\xBF]{3})*+#A',
                $input, $m);
    ...
    private function contextHtmlTag()
    {
        ...
        if (!empty($matches['end'])) { // end of HTML tag />
            ...
            $this->setContext(!$this->xmlMode && in_array($this->lastHtmlTag, ['script', 'style'],
                true) ? self::CONTEXT_HTML_CDATA : self::CONTEXT_HTML_TEXT);
     ...
     * Changes macro tag delimiters.
     *
     * @param string $left  left regular expression
     * @param string $right right regular expression
     *
     * @return static
     */
    ...
    {
        switch ($token->type) {
            case Token::MACRO_TAG:
                if ($token->name === '/syntax') {
                    $this->setSyntax($this->defaultSyntax);
                    $token->type = Token::COMMENT;
                } elseif ($token->name === 'syntax') {
                    $this->setSyntax($token->value);
                    $token->type = Token::COMMENT;
                } elseif ($token->name === 'contentType') {
            ...
            case Token::HTML_ATTRIBUTE_BEGIN:
                if ($token->name === 'n:syntax') {
                    $this->setSyntax($token->value);
            ...
            case Token::HTML_TAG_BEGIN:
                if ($this->lastHtmlTag === $this->syntaxEndTag) {
                    $this->syntaxEndLevel++;
                }
                break;

            case Token::HTML_TAG_END:
                if ($this->lastHtmlTag === ('/' . $this->syntaxEndTag) && --$this->syntaxEndLevel === 0) {
                    $this->setSyntax($this->defaultSyntax);
                }
                break;
        }
