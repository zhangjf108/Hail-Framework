...
use Hail\Latte\{
    Helpers, MacroInterface, Strict, Engine
};
...
class Compiler
{
    ...
    public function addMacro(string $name, MacroInterface $macro, $flags = null)
    {
        if (!isset($this->flags[$name])) {
            $this->flags[$name] = $flags ?: MacroInterface::DEFAULT_FLAGS;
    ...
    public function compile(array $tokens, $className): string
    {
        ...
        foreach ($tokens as $this->position => $token) {
            if ($this->inHead && !($token->type === $token::COMMENT
                    || $token->type === $token::MACRO_TAG && ($this->flags[$token->name] ?? null) & MacroInterface::ALLOWED_IN_HEAD
        ...
        while ($this->macroNode) {
            if (~$this->flags[$this->macroNode->name] & MacroInterface::AUTO_CLOSE) {
    ...
    {
        if ($this->lastAttrValue === '' && $this->context && 0 === strpos($this->context,
    ...
    {
        if ($this->context === self::CONTEXT_HTML_TAG || $this->context && 0 === strpos($this->context,
                self::CONTEXT_HTML_ATTRIBUTE)
        ) {
        ...
        if ($token->closing) {
            ...
            if (!$token->empty && ($this->flags[$token->name] ?? null) & MacroInterface::AUTO_EMPTY) {
                $pos = $this->position;
    ...
    private function processHtmlTagEnd(Token $token)
    {
        ...
        if ($htmlNode->closing) {
            ...
            && (!isset($htmlNode->attrs['type']) || preg_match('#(java|j|ecma|live)script|json|css#i',
                    $htmlNode->attrs['type']))
        ) {
    ...
    private function processHtmlAttributeBegin(Token $token)
    {
        if (0 === strpos($token->name, Parser::N_PREFIX)) {
            ...
            if ($this->macroNode && $this->macroNode->htmlNode === $this->htmlNode) {
    ...
    public function closeMacro(
        ...
        if (!$node || ($node->name !== $name && '' !== $name) || $modifiers
            ...
            $name = $nPrefix
                ? "</{$this->htmlNode->name}> for " . Parser::N_PREFIX . implode(' and ' . Parser::N_PREFIX,
                    array_keys($this->htmlNode->macroAttrs))
    ...
    public function expandMacro(string $name, string $args, string $modifiers = null, $nPrefix = null): MacroNode
    {
        ...
        if (strpbrk($name, '=~%^&_')) {
            if (in_array($this->context, [self::CONTEXT_HTML_ATTRIBUTE_URL, self::CONTEXT_HTML_ATTRIBUTE_UNQUOTED_URL],
                true)) {
                if (!Helpers::removeFilter($modifiers, 'nocheck') && !preg_match('#\|datastream(?=\s|\||\z)#i',
                        $modifiers)
                ) {
                    $modifiers .= '|checkurl';
                }
        ...
        if ($nPrefix === MacroNode::PREFIX_INNER) {
            if (!strcasecmp($this->htmlNode->name, 'script')) {
                $context = [$this->contentType, self::CONTEXT_HTML_JS];
            } elseif (!strcasecmp($this->htmlNode->name, 'style')) {
